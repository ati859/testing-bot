...
import Database from 'better-sqlite3'; // or use sqlite3 if you prefer
const sqlite = new Database('db.sqlite');

// Ensure the liked_tracks table exists
sqlite.prepare(`
...
export class PlayerManager {
  ...
  constructor(player) {
    ...
    this.textChannelId = player?.textId;
   ...
   * @param {KazagumoTrack|null} track - Optional track to play
  ...
  saveState() {
    try {
      ...
      db.savePlayerState(guildId, {
        ...
        textChannelId: this.player.textId,
...
const is247Enabled = client.db?.is247Enabled?.(this.player.guildId) || false;
  ...
  async trackFromHistory(track, fromHistory = true) {
    try {
      ...
      if (!playSuccess && this.player.shoukaku && convertedTrack.track) {
        try {
         
          await this.player.shoukaku.playTrack({ track: convertedTrack.track });
          playSuccess = true;
   ...
   */
  shuffle() {
    if (!this.player.queue || this.player.queue.length < 2) return;
    // Fisher-Yates shuffle
    for (let i = this.player.queue.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [this.player.queue[i], this.player.queue[j]] = [this.player.queue[j], this.player.queue[i]];
    }
...
// Like a track
db.user.likeTrack = async function(userId, trackUri, trackData) {
  // Clean the track data to avoid circular references
  const cleanData = typeof trackData === 'object' && trackData !== null && typeof trackData.title !== 'undefined'
    ? {
  ...
  const stmt = sqlite.prepare(`
    INSERT OR REPLACE INTO liked_tracks (user_id, track_uri, track_title, track_author, track_data)
    VALUES (?, ?, ?, ?, ?)
  `);
  stmt.run(
    userId,
...
// Unlike a track
db.user.unlikeTrack = async function(userId, trackUri) {
  const stmt = sqlite.prepare(`
    DELETE FROM liked_tracks WHERE user_id = ? AND track_uri = ?
  `);
  stmt.run(userId, trackUri);
};

// Check if a track is liked
db.user.isTrackLiked = async function(userId, trackUri) {
  const stmt = sqlite.prepare(`
    SELECT 1 FROM liked_tracks WHERE user_id = ? AND track_uri = ?
  `);
  const row = stmt.get(userId, trackUri);
  return !!row;
};

db.user.getLikedSongs = function(userId, limit = 10, offset = 0) {
  const stmt = sqlite.prepare(`
    SELECT track_data FROM liked_tracks
    WHERE user_id = ?
...
db.user.countLikedSongs = function(userId) {
  const stmt = sqlite.prepare(`
...
db.user.removeLikedSong = function(userId, trackUri) {
  const stmt = sqlite.prepare(`
    DELETE FROM liked_tracks WHERE user_id = ? AND track_uri = ?
  `);
  const info = stmt.run(userId, trackUri);
  return info.changes > 0;
};

db.user.clearLikedSongs = function(userId) {
  const stmt = sqlite.prepare(`
